        ...
        if (Array.isArray(res.result.output.text)) {
          ...
          for (let i = 0; i < res.result.output.generic.length; i++) {
            if ((res.result.output.generic[i].response_type = "text")) {
              if (res.result.output.generic[i].text != undefined) {
                ans += res.result.output.generic[i].text + "\n";
              } else {
                ans +=
                  res.result.output.generic[
                    res.result.output.generic.length - 1
                  ].title;
              }
            }
          }
          conversation_response = ans;
        }
